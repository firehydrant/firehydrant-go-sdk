...
```go
firehydrantgosdk.WithServerURL("https://api.example.com")
```
...
```go
firehydrantgosdk.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
firehydrantgosdk.WithServerIndex(1)
```
...
```go
firehydrantgosdk.WithClient(httpClient)
```
...
```go
firehydrantgosdk.WithSecuritySource(/* ... */)
```
...
```go
firehydrantgosdk.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
firehydrantgosdk.WithTimeout(30 * time.Second)
```
